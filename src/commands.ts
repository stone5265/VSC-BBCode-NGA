...
export function activateCommands(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        ...
        vscode.commands.registerCommand('bbcode-nga.replaceImg', replaceImgCommand),
        vscode.commands.registerCommand('bbcode-nga.tableToMarkdown', tableToMarkdownCommand)
    )
    // console.log('命令注册完毕')
}
...
const BBCODE2MD: { [key: string]: string } = {
    'b': '**',
    'i': '*',
    'del': '~~',
    'code': '`'
...
function _getDisplayWidth(text: string): number {
    let width = 0;
    for (let char of text) {
        const code = char.codePointAt(0) || 0
        if (
            (0x4E00 <= code && code <= 0x9FFF) ||  // CJK统一表意文字
...
function _fillUrl(url: string, img = false): string {
    if (img) {
...
class MarkdownTable {
    ...
    endPosition: vscode.Position | null = null
    rows: string[][] = []
    lenCols: { [key: number]: number } = {}
    row: string[] = []
    cell = ''
    curCol = 0
    buffer: { [key: string]: string | null } = {}
    ...
    updateCell(text: string, tag = '') {
        if (tag === 'tr') {
            ...
            if (this.buffer['url']) {
                const caption = text
                const url = _fillUrl(this.buffer['url'] as string)
    ...
    build(endPosition: vscode.Position): string {
        this.endPosition = endPosition
        let filledRows: string[] = []
...
async function tableToMarkdownCommand() {
    ...
    const processSelection = async (selection: vscode.Selection) => {
        ...
        let mdTable: MarkdownTable = dumTable
        let lastPosition = 0
